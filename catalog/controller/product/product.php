		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'orig'  => $this->model_tool_image->resize($result['image'], '247', '370'),
					'thumb_popup'  => $this->model_tool_image->resize($result['image'], '47', '73'),
				);
			}
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$data['packing_price'] = $this->currency->format($this->tax->calculate($product_info['packing_price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$data['$packing_price'] = false;
			}
			...
			//Вывод изображения для опции
			$data['image_option'] = array();
			...
			foreach($options as $option){
				if($option['option_id'] == 15){
					...
					foreach($option['product_option_value'] as $value){
						if($value['link_product_id'] == 0){
							$data['image_option'][] = array(
								'image' => $this->model_tool_image->resize($value['image'], '15', '15'),
								'link_product_id' => $this->url->link('product/product', $url . '&product_id=' . $this->request->get['product_id']) 
							);
						}else{
							$data['image_option'][] = array(
								'image' => $this->model_tool_image->resize($value['image'], '15', '15'),
								'link_product_id' => $this->url->link('product/product', $url . '&product_id=' . $value['link_product_id'])
							);
						}
					}
				}
