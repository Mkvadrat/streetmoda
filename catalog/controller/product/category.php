	...
	public function index() {
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$packing_price = $this->currency->format($this->tax->calculate($result['packing_price'], $result['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$packing_price = false;
				}
						...
						foreach($option['product_option_value'] as $value){
							if($value['link_product_id'] == 0){
								$image_option[] = array(
									...
									'link_product_id' => $this->url->link('product/product', $url . '&product_id=' . $result['product_id'])
								);
							}else{
								$image_option[] = array(
									'image' => $this->model_tool_image->resize($value['image'], '15', '15'),
